    ...
    if data_sign == 'train':
        ...
        for en_pair, rel in zip(example.en_pair_list, example.re_list):
            ...
            if sub_head != -1 and obj_head != -1:
                ...
                if ex_params['use_symmetries'] == 'symmetries':
                    corres_tag[obj_head][sub_head] = 1
                elif ex_params['use_symmetries'] == 'symmetries_rate':
                    corres_tag[obj_head][sub_head] = 0.8

        if ex_params['cross_data']:
            for c in range(ex_params['cross_data']):
                temp = corres_tag.copy()
                for i in range(max_text_len):
                    for j in range(max_text_len):
                        hidsum = temp[i]+temp[:, j]
                        hidden_count = len(np.where(hidsum > 1)[0])
                        corres_score = 1 - np.e ** -hidden_count + temp[i,j]
                        corres_tag[i,j] = min(1, corres_score)
